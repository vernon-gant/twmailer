...
UserContext AuthGuard::authenticate(const std::shared_ptr<ConnectionSocket> &connection_socket) const {
    ...
    if (_black_list_handler->is_banned(credentials.user_name, connection_socket->get_client_ip_address()))
        throw IsBanned("You are banned : please wait until your ban for " + credentials.user_name + " is released...");

    int times_banned = _black_list_handler->get_bans_amount(credentials.user_name,
                                                            connection_socket->get_client_ip_address());

    while (times_banned != 3) {
        ...
        times_banned = _black_list_handler->get_bans_amount(credentials.user_name,
                                                            connection_socket->get_client_ip_address());
        credentials = _credentials_reader->read(connection_socket);
        log_try(connection_socket->get_client_ip_address(), credentials.user_name);
    }

    throw GotBanned("Got banned : you got banned for 1 minute with " + credentials.user_name, credentials.user_name);
}
...
void AuthGuard::log_try(const std::string &client_address, const std::string &user_name) {
    std::cout << client_address << "  #############  Tried to login with \"" << user_name << "\"" << std::endl;
}
