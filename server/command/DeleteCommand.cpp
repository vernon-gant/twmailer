...
void DeleteCommand::execute() {
    const std::string &user_directory = FileSystemUtils::get_user_directory(_message_request.user_name);
    const std::string &file_path = user_directory + "/message_" + std::to_string(_message_request.message_number);
    try {
        if (std::filesystem::remove(file_path)) response = "OK\n";
        else response = "ERR\n";
...
DeleteCommand::DeleteCommand(const UserContext &userContext, SingleMessageRequest messageRequest) : Command(
