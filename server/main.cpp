...
int main(int argc, char **argv) {
    ...
    try {
        ...
        logger->log_fatal_error(std::string(error.what()));
    }
    ...
    std::unique_ptr<CommandVisitor> logging_visitor = std::make_unique<LoggingVisitor>(logger);
    ...
    std::unique_ptr<AuthGuard> auth_guard = std::make_unique<AuthGuard>(black_list_handler, credentials_reader,
                                                                        ldap_connector, logger);
    std::unique_ptr<ConnectionHandler> connection_handler = std::make_unique<ConnectionHandler>(
            std::move(command_parser), std::move(command_pipeline), std::move(auth_guard), logger);
    Server server(std::move(listening_socket), std::move(connection_handler), logger);
    server.start();
}