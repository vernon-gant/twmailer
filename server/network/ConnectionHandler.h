...
public:
    ConnectionHandler(std::unique_ptr<CommandFactory> command_parser, std::unique_ptr<CommandPipeline> command_pipeline, std::unique_ptr<AuthGuard> auth_guard)
            : _command_parser(std::move(command_parser)), _command_pipeline(std::move(command_pipeline)), _auth_guard(std::move(auth_guard)) {}
